...
def _deposit_section(dep: dict) -> list:
    ...
    if unmatched:
        ...
        for it in unmatched:
            name = _g(it, "depositor", "name", "입금자명", default="(이름없음)")
            amt = _won(it.get("amount") if isinstance(it, dict) else None)
            t = _g(it, "time", "시각")
...
def _lunch_section(lunch: dict) -> list:
    ...
    for t in order:
        ...
        for it in group:
            user = _g(it, "user", "payer", "결제자", default="(미상)")
            t_str = _g(it, "time", "시각")
            amt = _won(it.get("amount"))
...
def _non_lunch_section(nl: dict) -> list:
    ...
    for k, _ in buckets:
        for it in (nl.get(k) or []):
            try:
                total_amt += int(round(float(it.get("amount"))))
    ...
    for k, label in buckets:
        ...
        for it in rows:
            user = _g(it, "user", "결제자", default="(미상)")
            t_str = _g(it, "time", "시각")
...
def _attendance_section(att: dict) -> list:
    ...
    for k, label in blocks:
        ...
        for it in rows:
            any_row = True
            user = _g(it, "user", "name", default="(미상)")
            t_in = _g(it, "in", "attendanceTime", "출근")
            t_out = _g(it, "out", "quittingTime", "퇴근")
